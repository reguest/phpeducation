...
// Product ekleme
if (isset($_POST['add_sss_setting'])) {
    //echo "Güncelleme yap";
    extract($_POST);
    // print_r($_POST);

    $stmt = $config->db->prepare("INSERT INTO sss (title,description,status) VALUES (?,?,?)");
    $blogAdd = $stmt->execute(array($title,$description,$status));

    if ($blogAdd) {
        // echo "yes";
           header("Location:" . SITE_URL . "/panel/sss.php?status=ok");
    } else {
        echo "Sss ekleme sırasında hata oluştu!";
...
if (isset($_GET['delete'])) {
    ...
    $delete = $config->db->prepare("DELETE from sss where id=?");
    ...
    if ($execute) {
        ...
        // echo "no";
        header("Location:" . SITE_URL . "/panel/sss.php?status=no");
    }
